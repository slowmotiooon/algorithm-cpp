...
void dfs(vector<vector<int>>& heights, vector<vector<pair<bool, bool>>>& checked,
         vector<vector<bool>>& visited, int i, int j) {
    if (i == 0 || j == 0) checked[i][j].first = true;
    if (i == heights.size() - 1 || j == heights[0].size() - 1) checked[i][j].second = true;

    visited[i][j] = true;
    if (checked[i][j].first && checked[i][j].second) return;

    if (i != 0 && heights[i - 1][j] <= heights[i][j]) {
        if (!visited[i - 1][j]) dfs(heights, checked, visited, i - 1, j);
        if (checked[i - 1][j].first) checked[i][j].first = true;
        if (checked[i - 1][j].second) checked[i][j].second = true;
    }
    if (checked[i][j].first && checked[i][j].second) return;
    ...
    if (j != 0 && heights[i][j - 1] <= heights[i][j]) {
        if (!visited[i][j - 1]) dfs(heights, checked, visited, i, j - 1);
        if (checked[i][j - 1].first) checked[i][j].first = true;
        if (checked[i][j - 1].second) checked[i][j].second = true;
    }
    if (checked[i][j].first && checked[i][j].second) return;
...
vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
    vector<vector<pair<bool, bool>>> checked;
    vector<vector<bool>>             visited;
    for (int i = 0; i < heights.size(); i++) {
        vector<pair<bool, bool>> line1;
        vector<bool>             line2;
        for (int j = 0; j < heights[0].size(); j++) {
            line1.push_back({ false, false });
            line2.push_back(false);
        }
        checked.push_back(line1);
        visited.push_back(line2);
    }

    for (int i = 0; i < heights.size(); i++) {
        for (int j = 0; j < heights[0].size(); j++) {
            if (!visited[i][j]) dfs(heights, checked, visited, i, j);
        }
    }
    vector<vector<int>> ans;
    for (int i = 0; i < heights.size(); i++) {
        for (int j = 0; j < heights[0].size(); j++) {
            if (checked[i][j].first && checked[i][j].second) ans.push_back({ i, j });
        }
    }
    return ans;
}
